...
class InputEmbedding(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.input_size = self.patch_size * self.patch_size * self.n_channels # 16*16*3=768

        # Linear projection
        self.LinearProjection = nn.Linear(self.input_size, self.latent_size)
        # Class token
        self.class_token = nn.Parameter(torch.randn(self.batch_size, 1, self.latent_size).to(self.device)) # (4, 1, 768)
...
def main():
    ...
    parser.add_argument('--n-channels', type=int, default=1,
                        help='number of channels in images (default : 3 for RGB 1 for FashionMNist)')
    ...
    train_data = torchvision.datasets.FashionMNIST(root='./dataset', train=True, download=True, transform=transforms)
    valid_data = torchvision.datasets.FashionMNIST(root='./dataset', train=False, download=True, transform=transforms)
