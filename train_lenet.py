...
from network import LeNet5, LeNet6, LeNet7, LeNet8, LeNet9, AlexNet, AlexNet_dp2, AlexNet_dp7, VGG, vgg, LeNet5_bn,LeNet5_dp
...
def train(model_class, model_name):
    ...
    # model = model_class(model_name=model_name, num_classes=10, init_weights=True).to(device)
    ...
    for epoch in range(EPOCH):
        ...
        with tqdm(test_loader, unit="batch") as tepoch:
            for imgs, labels in tepoch:
                ...
                num_correct += (preds == labels).sum().item()
                loss = loss_fn(outputs, labels)
                test_loss += loss.item()   # 累加 batch loss
...
if __name__ == "__main__":
    models = [LeNet5,LeNet5_bn,LeNet5_dp]
